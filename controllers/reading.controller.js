...
//save the three cards that were generated from the API and save the three entrys 
exports.generateFiveReading = (req, res) => {
    const reading = new Reading({
        // entryId: req.body.entryId,
        date: new Date()
    })
    ...
    axios.get("https://rws-cards-api.herokuapp.com/api/v1/cards/random?n=5").then((resp) => {
        ...
        cards[3].save((err) => {
            console.log('Card Saved!', cards[3]._id)
        })
        cards[4].save((err) => {
            console.log('Card Saved!', cards[4]._id)
        })
        // res.send(cards)
    }).then(() => {
        reading.firstCard = cards[0]
        reading.secondCard = cards[1]
        reading.thirdCard = cards[2]
        reading.fourthCard = cards[3]
        reading.fifthCard = cards[4]
        //save reading
        reading.save(err => {
            if (err) {
                res.status(500).send({ message: err })
